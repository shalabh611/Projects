...
//        if (username!=null && username!="" && !username.equals("admin")) {
            ...
            if(customerDetails.getUserType().equals("Customer")) {
                ...
                List<Product> soldProductList = customerOrderService.getSoldItems(customerDetails.getCustomerId());
                System.out.println(" vendor - 2");
                System.out.println(" ---- "+soldProductList.size());
                System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                for(int i =0; i < soldProductList.size(); i++){
                    System.out.println(soldProductList.get(i));
                }
