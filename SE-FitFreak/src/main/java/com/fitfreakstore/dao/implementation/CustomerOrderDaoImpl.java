...
public class CustomerOrderDaoImpl implements CustomerOrderDao {
    ...
    @Override
    public List<Product> getSoldItems(int customerId) {

        Session session = sessionFactory.getCurrentSession();

        Query query = session.createSQLQuery("SELECT product.* from product , customerorder WHERE product.vendorid =:customerId and customerorder.productid = product.productid")
                .addEntity(Product.class).setParameter("customerId",customerId);
